...
checkboxes.forEach((checkbox) => {
  ...
  const itemIndex = [...checkbox.parentElement.parentElement.children].indexOf(
    checkbox.parentElement
  );

  if (checkbox.checked) {
    localStorage.removeItem(`${checklistName}_${itemIndex}`);
...
function copyToClipboard(exampleNumber) {
  const codeBlock = document.querySelector(
    `.code-snippet[data-example="${exampleNumber}"]`
  );
  if (codeBlock) {
